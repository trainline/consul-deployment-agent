...
class TestConsulApi(unittest.TestCase):
    ...
    def test_check_connectivity_succeeds(self):
        responses.add(responses.GET, 'http://localhost:8500/v1/agent/self', json={'some': 'content'}, status=200)
        consul_api = ConsulApi(consul_config)
        consul_api.check_connectivity()

    @patch('agent.consul_api.ConsulApi._api_get')
    ...
    def test_get_keys_for_existing_key_prefix(self):
        ...
        keys = ['key1', 'key2/key3']
        responses.add(responses.GET, 'http://localhost:8500/v1/kv/{0}'.format(key_prefix), json=keys, status=200)
        consul_api = ConsulApi(consul_config)
    ...
    def test_get_keys_for_unknown_key_prefix(self):
        key_prefix = 'keyprefix'
        responses.add(responses.GET, 'http://localhost:8500/v1/kv/{0}'.format(key_prefix), status=404)
        consul_api = ConsulApi(consul_config)
        actual_keys = consul_api.get_keys(key_prefix)
    ...
    def test_get_value_for_existing_key(self):
        ...
        value = [{'Key':key, 'Value':base64.b64encode(json.dumps(decoded_value)), 'LockIndex':0, 'CreateIndex':100, 'ModifyIndex':100, 'Flags':0}]
        responses.add(responses.GET, 'http://localhost:8500/v1/kv/{0}'.format(key), json=value, status=200)
        consul_api = ConsulApi(consul_config)
    ...
    def test_get_value_for_unknown_key(self):
        key = 'key'
        responses.add(responses.GET, 'http://localhost:8500/v1/kv/{0}'.format(key), status=404)
        consul_api = ConsulApi(consul_config)
        actual_value = consul_api.get_value(key)
    ...
    def test_get_service_catalogue(self):
        service_catalogue = { 'consul':{ 'Service':'consul', 'Tags':[], 'ModifyIndex':0, 'EnableTagOverride':False, 'ID':'consul','Address':'','CreateIndex':0, 'Port':8300}}
        responses.add(responses.GET, 'http://localhost:8500/v1/agent/services', json=service_catalogue, status=200)
        consul_api = ConsulApi(consul_config)
    ...
    def test_register_service_succeeds(self):
        responses.add(responses.PUT, 'http://localhost:8500/v1/agent/service/register', status=200)
        consul_api = ConsulApi(consul_config)
        is_success = consul_api.register_service(id='service_id', name='service_name', address='127.0.0.1', port=8080, tags=['tag'])
        self.assertEqual(is_success, True)

    @responses.activate
    def test_register_service_fails(self):
        responses.add(responses.PUT, 'http://localhost:8500/v1/agent/service/register', status=400)
        consul_api = ConsulApi(consul_config)
        is_success = consul_api.register_service(id='service_id', name='service_name', address='127.0.0.1', port=8080, tags=['tag'])
        self.assertEqual(is_success, False)
