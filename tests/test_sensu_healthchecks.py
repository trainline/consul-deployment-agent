...
class TestRegisterSensuHealthChecks(unittest.TestCase):
    ...
    def test_generate_windows_check_definition_with_command_and_no_arguments(self):
        ...
        check_definition = RegisterSensuHealthChecks.generate_check_definition(check, check['server_script'], self.deployment)
        self.assertEqual(check_definition['checks']['sensu-check1']['command'], 'powershell -noprofile -noninteractive -executionpolicy bypass -command "foo.ps1"')
    ...
    def test_generate_windows_check_definition_with_command_and_arguments(self):
        ...
        check_definition = RegisterSensuHealthChecks.generate_check_definition(check, check['server_script'], self.deployment)
        self.assertEqual(check_definition['checks']['sensu-check1']['command'], 'powershell -noprofile -noninteractive -executionpolicy bypass -command "C:\\Programs Files (x86)\\Sensu\\plugins\\check-windows-service.ps1" -ServiceName service_name')
    ...
    def test_generate_local_check_definition_with_command_and_arguments_and_slice(self):
        ...
        self.deployment.service.slice = 'green'
        check_definition = RegisterSensuHealthChecks.generate_check_definition(check, check['local_script'], self.deployment)
        self.assertEqual(check_definition['checks']['sensu-check1']['command'], 'powershell -noprofile -noninteractive -executionpolicy bypass -command "C:\\Programs Files (x86)\\Sensu\\plugins\\check-windows-service.ps1" -ServiceName service_name green')
