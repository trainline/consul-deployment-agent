...
from health_checks.consul_health_check import ConsulHealthCheck
        ...
        if deployment.last_id is None:
            ...
            if previous_appspec is None:
                deployment.logger.warning(
                    'Previous deployment directory not found, id: {0}'.format(deployment.last_id))
            else:
                (healthchecks, scripts_base_dir) = find_healthchecks(
                    'consul', deployment.last_archive_dir, previous_appspec, deployment.logger)
                if healthchecks is None:
                    return
                for check_id, check in healthchecks.iteritems():
                    service_check_id = create_service_check_id(
                        deployment.service.id, check_id)
                    is_success = deployment.consul_api.deregister_check(
                        service_check_id)
                    if is_success:
                        deployment.logger.info(
    ...
    def _run(self, deployment):
            ...
            for check_id, check in healthchecks.iteritems():
                ...
                if check['type'] == 'script':
                    ...
                    file_path = os.path.join(
                        deployment.archive_dir, scripts_base_dir, check['script'])
                    if not os.path.exists(file_path):
                        raise DeploymentError('Couldn\'t find health check script in package with path: {0}'.format(
        ...
        def validate_check(check_id, check):
            if not 'type' in check or (check['type'] != 'script' and check['type'] != 'http'):
                raise DeploymentError('Failed to register health check \'{0}\', only \'script\' and \'http\' check types are supported, found {1} .'.format(
                    check_id, check['type']))
            if check['type'] == 'script':
        ...
        (healthchecks, scripts_base_dir) = find_healthchecks('consul',
                                                             deployment.archive_dir, deployment.appspec, deployment.logger)
        if healthchecks is None:
            return
        ...
        for check_id, check in healthchecks.iteritems():
            ...
            if check['type'] == 'script':
                ...
                is_success = deployment.consul_api.register_script_check(
                    deployment.service.id, service_check_id, check['name'], file_path, check['interval'])
            elif check['type'] == 'http':
                is_success = deployment.consul_api.register_http_check(
                    deployment.service.id, service_check_id, check['name'], check['http'], check['interval'])
            else:
                is_success = False
